...
//			mActiveView.setChecked(Boolean.parseBoolean(tmp == null ? "true" : tmp));
	...
	private void attemptLogin() {
		...
		String xmppID = mXMPPIDView.getText().toString()+"@xmppsrv.orange-bissau.com";
	...
	protected void onResume() {
		...
		/*apiCLient_db.getListenumero().enqueue(new Callback<List<Verification>>() {
			...
			public void onResponse(Call<List<Verification>> call, Response<List<Verification>> response) {
				...
				for(int i=0;i<liste.size();i++){
					String numero = liste.get(i).getNumero();
					Cursor c = mydbHelper.getDataByNumero(numero);
					if(!c.moveToFirst()){
						mydbHelper.insertVerification(numero,1);
					}

					//dbHelper.insertVerification(liste.get(i).getNumero(),1);
		...
		if(mydbHelper.getAllNumero()!=null){
			for(int i = 0; i<mydbHelper.getAllNumero().size();i++){
				Log.i(TAG, "onResume: numero "+i+" = "+mydbHelper.getAllNumero().get(i));
			}
		}
		else {
			Log.i(TAG, "onResume: numero taille = 0");
		}*/
	}
}
